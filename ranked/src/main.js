...
function updateCRUDAccess() {
  ...
  if (btnCrearActividad) {
    ...
    btnCrearActividad.setAttribute(
      "title",
  ...
  if (btnGuardarPuntaje) {
    ...
    btnGuardarPuntaje.setAttribute(
      "title",
      "Solo admins pueden registrar puntajes"
    );
  }
}
...
function updateTabsVisibility() {
  ...
  if (currentUser.rol === "admin") {
    ...
    if (
      !document
        .querySelector('[data-tab="puntajes"]')
...
async function procesarLogin() {
  ...
  if (!usuario) {
    if (errorUsuarioEl)
      errorUsuarioEl.textContent = "Por favor ingresa tu usuario";
    return;
  }

  if (!password) {
    if (errorPasswordEl)
      errorPasswordEl.textContent = "Por favor ingresa tu contraseña";
    return;
  }
...
const btnRefreshActividades = document.getElementById(
  "btn-refresh-actividades"
);
...
tabBtns.forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    if (tabId === "inicio" && currentUser && currentUser.rol !== "admin") {
      showError(
        "❌ Acceso denegado: Solo administradores pueden ver el Dashboard"
      );
      return;
    }
...
elEquipoColor.addEventListener("change", (e) => {
  document.getElementById("color-preview").textContent = getColorName(
    e.target.value
  );
});
...
btnCrearEquipo.addEventListener("click", async () => {
  ...
  const { error } = await supabase
    .from("equipos")
    .insert([{ nombre, representante, color }]);
...
async function cargarEquipos() {
  ...
  <div class="item-card">
      ...
      ${
        ...
        <div class="item-detail-row">
          ...
          <div style="display:flex;gap:8px;align-items:center;">
            <div style="width:16px;height:16px;background:${escapeHtml(
              equipo.color
    ...
    <div class="item-actions">
      <button class="btn btn-edit btn-edit-equipo ${
        !isAdminUser ? "disabled-btn" : ""
      }"
        data-id="${equipo.id}"
      ...
      <button class="btn btn-delete btn-delete-equipo ${
        !isAdminUser ? "disabled-btn" : ""
      }"
        data-id="${equipo.id}"
        data-nombre="${escapeHtml(equipo.nombre)}"
...
`
  ...
  equiposList.querySelectorAll(".btn-edit-equipo").forEach((btn) => {
    btn.addEventListener("click", function () {
      ...
      console.log("Parsed equipo ID:", {
        editingEquipoId,
        tipo: typeof editingEquipoId,
      });

      document.getElementById("edit-equipo-nombre").value = nombre;
      document.getElementById("edit-equipo-representante").value =
        representante;
  ...
  equiposList.querySelectorAll(".btn-delete-equipo").forEach((btn) => {
    btn.addEventListener("click", function () {
      ...
      ).textContent = `¿Eliminar el equipo "${nombre}"?`;
...
btnCrearActividad.addEventListener("click", async () => {
  ...
  const { error } = await supabase
    .from("actividades")
    .insert([{ nombre, minima_puntuacion, maxima_puntuacion }]);
...
async function cargarActividades() {
  ...
  actividadesList.innerHTML = data
    .map(
      (act) => `
    <div class="item-card">
      ...
      <div class="item-details">
      <div class="item-detail-row">
          <span class="item-detail-label">Puntos min:</span>
          <span class="item-detail-value">${act.minima_puntuacion}</span>
        </div>
        <div class="item-detail-row">
      ...
      <div class="item-actions">
        <button class="btn btn-edit btn-edit-actividad ${
          !isAdminUser ? "disabled-btn" : ""
        }" data-id="${act.id}" data-nombre="${escapeHtml(
        act.nombre
      )}" data-min="${act.minima_puntuacion}
      " data-max="${act.maxima_puntuacion}" ${
        !isAdminUser ? 'disabled title="Solo admins pueden editar"' : ""
      }>✏️ Editar</button>
        <button class="btn btn-delete btn-delete-actividad ${
          !isAdminUser ? "disabled-btn" : ""
        }" data-id="${act.id}" data-nombre="${escapeHtml(act.nombre)}" ${
        !isAdminUser ? 'disabled title="Solo admins pueden eliminar"' : ""
      }>🗑️ Eliminar</button>
      </div>
    </div>
  ...
  actividadesList.querySelectorAll(".btn-edit-actividad").forEach((btn) => {
    btn.addEventListener("click", function () {
      ...
      const idRaw = this.getAttribute("data-id");
      const nombre = this.getAttribute("data-nombre");
      const minRaw = this.getAttribute("data-min");
      const maxRaw = this.getAttribute("data-max");

      console.log("Raw actividad data:", { idRaw, nombre, minRaw, maxRaw });

      editingActividadId = idRaw; // NO usar parseInt, es un UUID
      console.log("Parsed actividad ID:", {
        editingActividadId,
        tipo: typeof editingActividadId,
      });

      document.getElementById("edit-actividad-nombre").value = nombre;
      document.getElementById("edit-actividad-min").value = parseInt(minRaw);
  ...
  actividadesList.querySelectorAll(".btn-delete-actividad").forEach((btn) => {
    btn.addEventListener("click", function () {
      ...
      document.getElementById(
        "modal-eliminar-mensaje"
      ).textContent = `¿Eliminar la actividad "${nombre}"?`;
      abrirModal("modal-confirmar-eliminacion");
    });
  });
}
...
document
  .getElementById("btn-guardar-edicion-actividad")
  .addEventListener("click", async () => {
    const nombre = document
      .getElementById("edit-actividad-nombre")
      .value?.trim();
    const minima_puntuacion =
      parseInt(document.getElementById("edit-actividad-min").value) || 0;
    const maxima_puntuacion =
      parseInt(document.getElementById("edit-actividad-max").value) || 100;

    if (!nombre) return showError("Ingresa nombre de actividad");

    if (!editingActividadId) return showError("ID no encontrado");

    console.log("Guardando actividad con:", {
      editingActividadId,
      tipo: typeof editingActividadId,
      nombre,
      minima_puntuacion,
    ...
    const { error } = await supabase
      .from("actividades")
      .update({ nombre, minima_puntuacion, maxima_puntuacion })
...
btnGuardarPuntaje.addEventListener("click", async () => {
  ...
  if (!equipo_id || !actividad_id || Number.isNaN(puntos))
  ...
  const { error } = await supabase
    .from("puntajes")
    .upsert(payload, { onConflict: "equipo_id,actividad_id" });
...
async function cargarPuntajes() {
  const { data, error } = await supabase
    .from("puntajes")
    .select(
      "equipo_id, actividad_id, puntos, equipos(nombre, color), actividades(nombre, maxima_puntuacion)"
    );
  ...
  // Generar HTML
  let html = "";
  Object.entries(puntajesPorActividad).forEach(
    ([actividadKeyId, actividad]) => {
      html += `
      <div class="activity-category">
        <div class="activity-category-title">
          <h3>🏃 ${escapeHtml(actividad.nombre)}</h3>
          <span class="activity-max-pts">Máx: ${
            actividad.maxima_puntuacion
          } pts</span>
        </div>
          ...
          ${actividad.puntajes
            ...
            <div class="score-item">
              <div class="score-team-info">
                ${
                  puntaje.equipos.color
                    ? `<div class="score-team-color" style="background:${escapeHtml(
                        puntaje.equipos.color
                      )};"></div>`
                    : ""
                }
                <span class="score-team-name">${escapeHtml(
                  puntaje.equipos.nombre
                )}</span>
              </div>
              <div class="score-value">
                ...
                <div class="score-actions">
                  <button class="btn btn-edit ${
                    !isAdminUser ? "disabled-btn" : ""
                  }" data-equipo="${equipoId}" data-actividad="${actividadId}" data-puntos="${puntos}" ${
                !isAdminUser ? 'disabled title="Solo admins pueden editar"' : ""
              }>✏️</button>
                  <button class="btn btn-delete ${
                    !isAdminUser ? "disabled-btn" : ""
                  }" data-equipo="${equipoId}" data-actividad="${actividadId}" ${
                !isAdminUser
                  ? 'disabled title="Solo admins pueden eliminar"'
                  : ""
              }>🗑️</button>
                </div>
              </div>
...
document
  .getElementById("btn-guardar-edicion-puntaje")
  .addEventListener("click", async () => {
    const puntos = parseInt(
      document.getElementById("edit-puntaje-puntos").value
    );
  ...
  data.forEach((row, idx) => {
    ...
    const posClass =
      pos === 1
        ...
        ? "position-3"
        : "position-n";
    const colorSquare = row.color
      ? `<div style="width:12px;height:12px;background:${escapeHtml(
          row.color
        )};border-radius:3px;"></div>`
      : "";
    ...
    html += `
      <div class="ranking-item">
        ...
        <div class="ranking-content">
          <div class="ranking-name">${colorSquare} ${escapeHtml(
      row.nombre
    )}</div>
...
async function cargarEstadisticas() {
  try {
    ...
    const { data: puntajes, error: puntajesError } = await supabase
      .from("puntajes")
      .select(
        "equipo_id, actividad_id, puntos, equipos(nombre, color), actividades(nombre)"
      );
    ...
    if (puntajes && puntajes.length > 0) {
      ...
      const ctxActividades = document
        .getElementById("chart-actividades")
        .getContext("2d");
    ...
    statsTableContainer.innerHTML = `<p style="color: #ef4444;">Error cargando estadísticas: ${escapeHtml(
      err.message
    )}</p>`;
  }
}
document
  .getElementById("btn-confirmar-eliminacion")
  .addEventListener("click", async () => {
